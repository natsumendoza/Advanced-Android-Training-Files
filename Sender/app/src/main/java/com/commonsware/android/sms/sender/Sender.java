  ...
  public void sendTheMessage(View v) {
    ...
    if (means.getCheckedRadioButtonId()==R.id.client) {
      ...
      if (msg.getText().length() > 160) {
        ArrayList<String> messageList = SmsManager.getDefault().divideMessage(msg.getText().toString());
        SmsManager.getDefault().sendMultipartTextMessage(c.getString(2), null, messageList, null, null);
      } else {
        SmsManager
                .getDefault()
